...
public class ImpactController : MonoBehaviour {
	...
	private void SetupAnimation(AnimationClip impactAnimation) {
		GetComponent<Animation>()[impactAnimation.name].wrapMode = WrapMode.Once;
		GetComponent<Animation>()[impactAnimation.name].blendMode = AnimationBlendMode.Additive;	
		GetComponent<Animation>()[impactAnimation.name].layer = 100;
	}
    ...
    weight *= 10000.0f;
		if (weight > 0) {
			AnimationState a = GetComponent<Animation>()[impactAnimation.name];
