...
public class WeaponStateController : MonoBehaviour {
	...
	bool inReload { get { return GetComponent<Animation>()[reloadAnimation.name].enabled; } }
	bool inTransition { get { return GetComponent<Animation>()[gunOffAnimation.name].enabled || GetComponent<Animation>()[gunOnAnimation.name].enabled;	} }
	bool inFire { get { return GetComponent<Animation>()[fireAnimation.name].enabled; } }
	
	// Set up upper body animation
	private void SetupGunAnimation (AnimationClip animationClip) {
		AnimationState a = GetComponent<Animation>()[animationClip.name];
	...
	void Start () {
		...
		AnimationState fireAS = GetComponent<Animation>()[fireAnimation.name];
	...
	void HolsterGun () {
		if (gunOn && !inReload && !inFire) {
			GetComponent<Animation>().CrossFade(noAimLocomontionGroup); 
			GetComponent<Animation>().CrossFade(gunOffAnimation.name);
			gunOn = false;
	...
	void UnHolsterGun () {
		if (!gunOn && !inReload && !inFire) {
			GetComponent<Animation>().CrossFade(aimLocomotionGroup); 
			GetComponent<Animation>().CrossFade(gunOnAnimation.name); 
			gunOn = true;
	...
	void ReloadGun () {
		...
		if (!gunOn) {
			...
			GetComponent<Animation>().CrossFade(reloadAnimation.name);
			bullets = clipSize;
